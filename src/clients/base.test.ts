...
describe('BaseClient', () => {
  ...
  describe('get method - error cases', () => {
    it('should throw RateLimitExceededError for 429 status', async () => {
      ...
      fetchSpy.mockReturnValueOnce(Promise.resolve({
        ...
        json: () => Promise.resolve({
          "error": {
            ...
            "message": "You have exceeded your rate limit. Try again later or contact us at https://api.congress.gov:443/contact/ for assistance"
          }
        }),
        headers: new Headers({
          'x-ratelimit-limit': '5000',
      ...
      await expect(client.getProxy('/committee/senate/rate-limit/nominations', { limit: 5 }))
        .rejects
    ...
    it('should throw CongressGovApiError for 404 error', async () => {
      ...
      await expect(client.getProxy('/amendment/117/SAMDT'))
        .rejects
        .toThrow(CongressGovApiError);
    });
  });
  ...
  describe('URL construction', () => {
    it('should construct fetch request with base endpoint', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(new URL("https://api.congress.gov/v3/bill/117"), {
        headers: {
    ...
    it('should properly construct fetch request with query parameters', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(new URL("https://api.congress.gov/v3/bill/117?limit=5"), { "headers": { "Content-Type": "application/json", "X-API-Key": "test-api-key" } })
    });
  });
});